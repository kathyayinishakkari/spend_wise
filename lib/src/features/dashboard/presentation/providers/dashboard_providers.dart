...
final reimbursementsStateProvider = StateProvider<List<Reimbursement>>((ref) => const []);

final dashboardSummaryProvider = Provider<AsyncValue<DashboardSummary>>((ref) {
  final expensesAsync = ref.watch(expensesProvider);
  final budget = ref.watch(selectedBudgetProvider);
  final reimbursements = ref.watch(reimbursementsStateProvider);

  return expensesAsync.whenData((expenses) {
    final now = DateTime.now();
    final monthly = expenses.where((e) => e.dateTime.year == now.year && e.dateTime.month == now.month).toList();
    final spend = monthly.fold<double>(0, (sum, e) => sum + e.amount);
    final budgetAmount = budget?.amount ?? 0;
    final pending = reimbursements.fold<double>(0, (sum, r) => sum + r.pendingAmount);
    ...
    return DashboardSummary(
      ...
      utilization: budgetAmount <= 0 ? 0 : (spend / budgetAmount).clamp(0, 1),
